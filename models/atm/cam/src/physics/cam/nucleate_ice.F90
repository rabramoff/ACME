...
!
! Method:
!  The current method is based on Liu & Penner (2005)
...
!
!  Treatment for prexisiting ice has been added.
! 
!  Mixed phase ice nucleation now follows Hoose et al (2010).
!
! Authors:
!  Xiaohong Liu, 01/2005, modifications by A. Gettelman 2009-2010
!  Xiaohong Liu and Xiangjun Shi for pre-existing ice (Shi et al. ACPD2014) 
!               with help from C. C. Chen and B. Eaton (2014)
!  Kai Zhang, 11/2014, implementation of pre-existing ice treatment in ACME v0.1 
!                      search KZ_PDFW_PREICE for details 
!-------------------------------------------------------------------------------
...
subroutine nucleati_init( &
   use_preexisting_ice_in, iulog_in, pi_in, mincld_in)
...
! 
! Xiangjun Shi, 2014, added pre-existing ice treatment 
! Kai Zhang,    2014, implemented in ACME v0.1
! 
! Reference:  
...
!!== KZ_PDFW_PREICE
   ...
   if (use_preexisting_ice) then
      ...
      if (Ni_preice > 10.0_r8) then    ! > 0.01/L   
         ! hom Si, if Shom<1 , Shom will be recalculated in SUBROUTINE Vpreice, based on Shom
         Shom = -1.5_r8
