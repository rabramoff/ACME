...
!-------------------------------------------------------------------------------
 ...
 !----------------------------------------------------------------------
  ...
  do j = lbj, ubj
    do p = 1, nactpft
      this%summseca(c_l,j)%competECA%mumax_minn_nh4_plant(p) = 1.e-7_r8  !plantNutkinetics%plant_nh4_vmax_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%mumax_minn_no3_plant(p) = 2.e-7_r8  !plantNutkinetics%plant_no3_vmax_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%mumax_minp_plant(p) = 4.e-8_r8      ! plantNutkinetics%plant_p_vmax_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%kaff_minn_no3_plant(p)= 1._r8       !plantNutkinetics%plant_no3_km_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%kaff_minn_nh4_plant(p)= 7.e-1_r8    !plantNutkinetics%plant_nh4_km_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%kaff_minp_plant(p)   = 6.e-1_r8     !plantNutkinetics%plant_p_km_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%plant_froot_nn(p) = 4._r8           !plantNutkinetics%plant_eff_ncompet_b_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%plant_froot_np(p) = 1000._r8        !plantNutkinetics%plant_eff_pcompet_b_vr_patch(p,j)
    enddo
    !affinity parameters
    !decomposers
    this%summseca(c_l,j)%competECA%kaff_minn_nh4_mic= 0.18_r8 !plantNutkinetics%km_decomp_nh4_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%kaff_minn_no3_mic= 0.41_r8 !plantNutkinetics%km_decomp_no3_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%kaff_minp_mic=  0.2_r8 !plantNutkinetics%km_decomp_p_vr_col(c_l,j)

    !nitrifiers and denitrifiers
    this%summseca(c_l,j)%competECA%kaff_minn_nh4_nit= 0.76_r8 !plantNutkinetics%km_nit_nh4_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%kaff_minn_no3_den= 0.11_r8 !plantNutkinetics%km_den_no3_vr_col(c_l,j)
    !mineral surfaces
    this%summseca(c_l,j)%competECA%kaff_minn_nh4_msurf= 3.e-1_r8     !plantNutkinetics%km_minsurf_nh4_vr_col(c_l,j)  !this is ignored at this moment
    this%summseca(c_l,j)%competECA%kaff_minp_msurf= 20._r8     !plantNutkinetics%km_minsurf_p_vr_col(c_l,j)

    !effective p competing decomposers
    this%summseca(c_l,j)%competECA%compet_bn_mic = 1._r8 !plantNutkinetics%decomp_eff_ncompet_b_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%compet_bp_mic = 1._r8 !plantNutkinetics%decomp_eff_pcompet_b_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%compet_bn_den = 1._r8 !plantNutkinetics%den_eff_ncompet_b_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%compet_bn_nit = 1._r8 !plantNutkinetics%nit_eff_ncompet_b_vr_col(c_l,j)

    this%summsforc(c_l,j)%msurf_nh4 = 1._r8 !plantNutkinetics%minsurf_nh4_compet_vr_col(c_l,j)   !this  number needs update
    this%summsforc(c_l,j)%msurf_minp= 1._r8 !plantNutkinetics%minsurf_p_compet_vr_col(c_l,j)    !this  number needs update
  enddo
  ...
  subroutine calc_bgc_reaction(this, bounds, col, lbj, ubj, num_soilc, filter_soilc, &
    ...
    write(*,*),'this%summsforc(c,j)%ystates(:)',this%summsforc(c,j)%ystates(:)
...
write(*,*)'this%summsforc(c,j)%ystates(litr_beg:litr_end)',this%summsforc(c,j)%ystates(litr_beg:litr_end)
write(*,*)'litr_beg',litr_beg
write(*,*)'litr_end',litr_end
write(*,*)'tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_litr:betrtracer_vars%id_trc_end_litr)',&
tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_litr:betrtracer_vars%id_trc_end_litr)
write(*,*)'betrtracer_vars%id_trc_beg_litr',betrtracer_vars%id_trc_beg_litr
write(*,*)'betrtracer_vars%id_trc_end_litr',betrtracer_vars%id_trc_end_litr
write(*,*)'c',c
write(*,*)'j',j
